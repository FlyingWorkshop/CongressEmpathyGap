...
class Corpus:
    ...
    def process(self, documents: list[Type[Document]], unigram_vocab: set, bigram_vocab: set):
        for doc in documents:
            if doc.label != self.label:
                continue
            self.num_docs += 1
            _ngram_count(doc.tokens, self.unigram_counts, unigram_vocab)
            _ngram_count(bigrams(doc.tokens), self.bigram_counts, bigram_vocab)
    ...
    def _train_bigram_likelihoods(self, corpora_bigram_vocab: set):
        ...
        for bigram, bigram_count in self.bigram_counts:
            unigram_count = self.unigram_counts[bigram[0]]
            self.bigram_likelihoods = (1 + bigram_count) / (unigram_count + v)

    def ngram_score(self, ngrams, likelihoods):
...
def _ngram_count(ngrams, count: dict, vocab: set):
    for ngram in ngrams:
