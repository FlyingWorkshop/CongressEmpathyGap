...
def _load_congress(dirname, keep_third_parties=False):
    documents = []
    for fname in os.listdir(dirname):
        ...
        for politician in roster:
            ...
            if party == Party.OTHER and not keep_third_parties:
                continue
            documents.append(CongWikiBio(name, party, politician["twitter_account"], politician["dw_nominate"]))
    return documents


def _load_governors(file, timespan, keep_third_parties=False):
    documents = []
    with open(file) as f:
        reader = csv.reader(f)
        next(reader)
        ...
        for row in reader:
            ...
            if name in seen or outdated(term, timespan) or (party == Party.OTHER and not keep_third_parties):
                continue
